...
public class OrbSystem {
    ...
    fun void init( float xr[], float yr[], int ol ) {
        //<<< "os_init", me.id(), "" >>>;
        xr[0] => X_RANGE[0];
        xr[1] => X_RANGE[1];
        yr[0] => Y_RANGE[0];
    ...
    fun void create_orb( float m, float x, float y ) {
        //<<< "os_create", me.id(), "" >>>;
        if( orbs.size() + 1 <= ORB_LIMIT ) {
            orbs.size( orbs.size() + 1 );
            new Orb @=> orbs[ orbs.size() - 1 ];
            orbs[ orbs.size() - 1 ].init( orbs.size() - 1, m, x, y, 0, 0 );
        }
    }
    ...
    fun void edge_check( Orb orb ) {
        //<<< "os_edge_check", me.id(), "" >>>;
        if( orb.loc[0] <= X_RANGE[0] + orb.m ) {
            -0.9 *=> orb.vel[0];
            X_RANGE[0] + orb.m => orb.loc[0];
        }
        if( orb.loc[0] >= X_RANGE[1] - orb.m ) {
            -0.9 *=> orb.vel[0];
            X_RANGE[1] - orb.m => orb.loc[0];
        }

        if( orb.loc[1] <= Y_RANGE[0] + orb.m ) {
            -0.9 *=> orb.vel[1];
            Y_RANGE[0] + orb.m => orb.loc[1];
        }
        if( orb.loc[0] >= Y_RANGE[1] - orb.m ) {
            -0.9 *=> orb.vel[1];
            Y_RANGE[1] - orb.m => orb.loc[1];
        }
    }
    ...
    fun Orb combine( Orb r_orbs[] ) {
        float x_mean, y_mean, m_tot;
        int new_id;
        for( int i; i < r_orbs.size(); i++ ) {
            r_orbs[i].loc[0] +=> x_mean;
            r_orbs[i].loc[1] +=> y_mean;
            r_orbs[i].m +=> m_tot;
        }
        r_orbs.size() /=> x_mean;
        r_orbs.size() /=> y_mean;
        ...
        Orb out;
        out.init( new_id, m_tot, x_mean, y_mean, 0, 0 );
        return out;
    }
